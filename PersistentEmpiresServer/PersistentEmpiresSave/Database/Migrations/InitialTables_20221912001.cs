    ...
    public class InitialTables_20221912001 : Migration
    {
        ...
        public override void Up()
        {
            ...
            Create.Table("users")
                   ...
                   .WithColumn("Administrator").AsBoolean()
                   .WithColumn("CreatedAt").AsDateTime().Nullable()
                                           .WithDefault(SystemMethods.CurrentDateTime)
                   .WithColumn("RoleId").AsInt32();
            Create.Table("discordconfig")
                .WithColumn("SettingName").AsString()
                .WithColumn("SettingValue").AsString()
                .WithColumn("SettingType").AsInt16();
            Create.Table("discordtickets")
                .WithColumn("TicketID").AsInt32().PrimaryKey().Identity()
                .WithColumn("ChannelId").AsString().Unique()
                .WithColumn("ParentId").AsString().Nullable()
                .WithColumn("MessageId").AsString().Nullable()
                .WithColumn("IssuerId").AsString().Nullable()
                .WithColumn("ClosedById").AsString().Nullable()
                .WithColumn("CreatedAt").AsDateTime().Nullable()
                                           .WithDefault(SystemMethods.CurrentDateTime)
                .WithColumn("ClosedAt").AsDateTime().Nullable();
            Create.Table("panelconfigs")
                .WithColumn("ConfigName").AsString().PrimaryKey()
                .WithColumn("ConfigValue").AsString();
            Create.Table("panelroles")
                .WithColumn("Id").AsInt32().PrimaryKey().Identity()
                .WithColumn("Name").AsString().Unique()
                .WithColumn("Permissions").AsCustom("JSON");
            ...
            Create.Table("whitelistAnswers")
                 .WithColumn("Id").AsInt32().PrimaryKey().Identity()
                  .WithColumn("whitelistQuestionsId").AsInt32()
                  .WithColumn("Answer").AsString()
                  .WithColumn("DiscordId").AsString();
            Create.Table("panellogs")
                .WithColumn("Id").AsInt32().PrimaryKey().Identity()
                .WithColumn("CreatedAt").AsDateTime().Nullable()
                .WithColumn("LogMessage").AsString()
                .WithColumn("ActionType").AsString()
                .WithColumn("Username").AsString().Nullable()
                .WithColumn("usersId").AsInt32().Nullable();
            Create.Table("playermodlogs")
                .WithColumn("Id").AsInt32().PrimaryKey().Identity()
                .WithColumn("CreatedAt").AsDateTime().Nullable()
                .WithColumn("PlayerId").AsString()
                .WithColumn("PlayerName").AsString()
                .WithColumn("PunishmentType").AsString()
                .WithColumn("DurationMin").AsInt32()
                .WithColumn("Reason").AsString()
                .WithColumn("BannedBy").AsString();
        }
    }
}
